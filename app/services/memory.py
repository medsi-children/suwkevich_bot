...
async def _refresh_profile_from_request(
    ...
    *,
    ...
    prompt = (
        ...
        '  "miniapp_lifehacks": [\n'
        '    {"title": "Лайфхак", "text": "лайфхак без заголовка", "action": "один шаг"}\n'
        "  ],\n"
        '  "miniapp_insights": [\n'
        ...
        "profile_summary пиши как короткое бережное описание, обращенное прямо к человеку. "
        "Говори во втором лице, лучше на «ты», если пользователь явно не просил иное. "
        "Не называй человека по имени в начале, не используй третье лицо и не пиши "
        "формулировки вроде «Денис любит...», «пользователь склонен...». Лучше: "
        "«Ты любишь...», «Тебе важно...», «Ты чаще...». Тон должен быть теплым, "
        "человечным и конкретным, без сухости, без ярлыков и без диагноза. "
        "В лайфхаках title всегда должен быть ровно «Лайфхак»; сам смысл пиши "
        "в text и action. Text — 1-2 коротких законченных предложения до 140 символов, "
        "action — один законченный шаг до 85 символов. Они должны быть человеческими, "
        "персональными, без канцелярита и странных фраз. "
...
async def generate_lifehack_for_profile(
    ...
    *,
    ...
    prompt = (
        ...
        "Верни только JSON без markdown:\n"
        '{ "title": "Лайфхак", "text": "человеческий совет без заголовка", '
        '"action": "один конкретный шаг" }\n\n'
        "Заголовок не придумывай: в title всегда ставь ровно «Лайфхак». "
        ...
        "В text дай 1-2 коротких законченных предложения до 140 символов, "
        "в action — один понятный законченный шаг до 85 символов без назидания.\n\n"
...
async def store_memory_updates(
    ...
    *,
    ...
    prompt = (
        ...
        "а не как для его работы. Формулируй естественным живым русским языком, без "
        "канцелярита, без искусственных \"психологических\" фраз и без кривых оборотов. "
        "Не пиши конструкции вроде \"ты сейчас громко\", \"включи эту фразу\", "
        "\"внутренний голос\", \"нейтральная формулировка\", если это звучит не по-русски. "
        "Не давай мантры, аффирмации, готовые реплики для самовнушения и советы в кавычках. "
        "Не отправляй пользователя в конкретные приложения, заметки, таймеры, файлы или "
        "другие интерфейсы телефона. Лайфхак должен звучать как короткий человеческий совет, "
        "на который не стыдно наткнуться в хорошей книге или у живого терапевта. "
        "Можно опираться на известные приемы из доказательных подходов, но пересказывай их "
        "своими словами, не цитируй дословно сайты и книги. "
        "Каждый пункт: максимум 2 короткие фразы, без списков и без вопросов.\n\n"
        ...
        "profile_summary тоже пиши во втором лице: это короткое описание, адресованное "
        "самому человеку. Предпочитай «Ты...» или, если стиль общения уже формальный, «Вы...». "
        "Не используй имя человека как подлежащее и не описывай его со стороны. "
