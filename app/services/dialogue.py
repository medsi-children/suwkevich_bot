...
def start_reply(first_name: str | None = None) -> str:
    ...
    return (
        ...
        "Я здесь, чтобы спокойно и по-человечески помогать разбираться в том, "
        ...
        "обо всем подряд, а на клинически аккуратный разговор о психике. "
        "В мою логику заложена ориентация на опыт сильных врачей и специалистов, "
        ...
        "красиво, а помогать мыслить точнее: отделять эмоции от фактов, замечать "
        ...
        "и безопасным.\n\n"
        "Я могу поддержать, задать уточняющие вопросы, помочь подготовиться "
        "к разговору с врачом или психотерапевтом, сформулировать наблюдения "
        "и увидеть варианты действий. При этом я не заменяю очного специалиста "
        "и не назначаю лечение, но могу помочь понять, что стоит обсудить "
        "с врачом.\n\n"
        "Напишите, пожалуйста, с чем вы пришли сейчас. Что с вами происходит "
        "в последние часы или дни?"
    )
...
def fallback_reply(text: str, risk_level: str) -> str:
    if not text.strip():
        ...
        base = (
            "Я вас услышал. Сейчас можно начать с трех вещей: что именно произошло, "
            "что вы чувствуете телом и эмоциями, и чего вы боитесь больше всего "
            "в ближайший час.\n\n"
            "Опишите это коротко, без попытки звучать правильно."
        )
